class Solution {
    ...
    private String helper(int num) {
        ...
        if (num < 100) return belowHundred[num / 10] + (num % 10 != 0 ? " " + helper(num % 10) : "");
        if (num < 1000) return helper(num / 100) + " Hundred" + (num % 100 != 0 ? " " + helper(num % 100) : "");
        if (num < 1000000) return helper(num / 1000) + " Thousand" + (num % 1000 != 0 ? " " + helper(num % 1000) : "");
        if (num < 1000000000) return helper(num / 1000000) + " Million" + (num % 1000000 != 0 ? " " + helper(num % 1000000) : "");
